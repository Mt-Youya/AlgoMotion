...
export function AlgoVideo({ config }: AlgoVideoProps) {
  ...
  return (
    <AbsoluteFill style={{ background: P.paper, fontFamily: "'Arial', sans-serif" }}>
      ...
      {inChapter("complexity") && ch.complexity && (
        <AbsoluteFill style={{ padding: "120px 100px 80px" }}>
          ...
          <div style={{ marginTop: 100, display: "flex", gap: 60 }}>
            <div>
              <h3 style={{ fontSize: 28, fontWeight: 750, margin: "0 0 18px", color: P.gray }}>
                TIME COMPLEXITY
              </h3>
              <ComplexityTable
                frame={frame}
                startFrame={21_640}
                rows={ch.complexity.timeRows}
              />
            </div>
            <div>
              <h3 style={{ fontSize: 28, fontWeight: 750, margin: "0 0 18px", color: P.gray }}>
      ...
      {!inChapter("intro") && (
        <BrandBadge frame={frame} startFrame={1_800} />
      )}
    ...
    <AbsoluteFill style={{ padding: "120px 80px 80px" }}>
      ...
      <div style={{ position: "absolute", top: 60, right: 72 }}>
        ...
        <div style={{
          ...
          <span style={{ fontSize: 22, color: P.gray }}>
      ...
      <div style={{ marginTop: 90 }}>
        <ContentBlock frame={frame} startFrame={startFrame + currentStep * framesPerStep}>
          <h2 style={{ fontSize: 48, fontWeight: 800, margin: "0 0 20px", color: P.ink }}>
            {step.label}
          </h2>
          <p style={{ fontSize: 28, color: P.gray, lineHeight: 1.5, maxWidth: 900 }}>
            {step.description}
          </p>
        </ContentBlock>
...
function CodeSection({ frame, startFrame, snippet, annotations }: CodeSectionProps) {
  const elapsed = frame - startFrame
  const framesPerAnnotation = annotations.length > 0
    ? Math.floor(5_360 / annotations.length)
    : 5_360
  const activeAnnotation = Math.min(
    Math.floor(elapsed / framesPerAnnotation),
